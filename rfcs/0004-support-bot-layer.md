...
6. Поддержать большой набор иерархических инструкций через tool-driven navigation без загрузки всего дерева в контекст.
...
6. Не реализовывать approval / permission layer в v1.
...
3. даёт LLM доступ к `InstructionRepository` как tool для навигации по дереву знаний;
4. собирает LLM input: system prompt, compact state, thread context;
...
Для v1 не используем скрытый auto-selection. `InstructionRepository` предоставляется модели как tool: LLM сначала листает дерево знаний, затем загружает одну или несколько конкретных статей.
...
2. позволяет получить список элементов на любом уровне дерева;
3. для каждого элемента возвращает `has_children`, чтобы LLM понимала, можно ли идти глубже;
4. загружает полный Markdown для списка выбранных ids;
5. применяет лимиты `max_load_documents` и `max_instruction_bytes`.
...
Human approval не входит в v1 execution loop. Если позже появятся tools, требующие подтверждения, они должны использовать асинхронную approval queue, а не блокировать основной LLM loop напрямую.

Планируемая модель:

1. LLM вызывает tool как обычный tool call.
2. `SupportBotHandler` получает вызов и через registry/policy определяет, что tool требует подтверждения.
3. Вызов сохраняется в отдельную асинхронную очередь pending approvals.
4. В engineer thread отправляется запрос на подтверждение с описанием tool call и аргументов.
5. Основной workflow не обязан ждать инженера синхронно.
6. Если инженер подтверждает запрос, tool выполняется так, будто его только что вызвала LLM.
7. Если инженер отклоняет запрос, создаётся rejected result.
8. В обоих случаях результат возвращается в контекст модели как обычный tool result на следующем продолжении обработки.

Очередь подтверждений должна быть отдельной подсистемой, потому что она живёт дольше одного `handle()` run и управляется внешними событиями из engineer channel.
...
- реализовать `InstructionRepository` как `SupportTool` с actions `list` и `load`;
...
- tree navigation по parent id;
- загрузка нескольких выбранных instruction ids одним tool call;
...
1. Нужен ли позже отдельный search/embedding tool поверх manifest, если ручной навигации по дереву станет недостаточно.
2. Как хранить pending approvals и как доставлять их результат обратно в LLM context.
3. Какой минимальный shape Mattermost interactive approval должен быть, если появятся tools с подтверждением.
4. Нужен ли отдельный trace sink помимо message metadata для отладки больших LLM/tool payloads.
...
8. Инструкции — custom documented filesystem format с manifest/tree; LLM загружает статьи через `InstructionRepository` tool.
...
11. Future approval должен строиться как асинхронная queue: LLM вызывает tool, bot ставит вызов в pending approvals, engineer thread получает запрос, а подтверждение/отклонение позже возвращается модели как tool result.
