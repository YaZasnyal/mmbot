...
pub async fn create_upload(configuration: &configuration::Configuration, create_upload_request: models::CreateUploadRequest) -> Result<models::UploadSession, Error<CreateUploadError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/api/v4/uploads", local_var_configuration.base_path);
...
pub async fn get_upload(configuration: &configuration::Configuration, upload_id: &str) -> Result<(), Error<GetUploadError>> {
    ...
    let local_var_uri_str = format!("{}/api/v4/uploads/{upload_id}", local_var_configuration.base_path, upload_id=crate::apis::urlencode(upload_id));
...
pub async fn upload_data(configuration: &configuration::Configuration, upload_id: &str) -> Result<models::FileInfo, Error<UploadDataError>> {
    let local_var_configuration = configuration;
